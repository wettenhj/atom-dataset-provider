...
var getIpAddress = function() {
  ...
  // This for loop could generate multiple IP addresses,
  // but we'll just return the first one we find:
  ...
  getOwnerOfFile(_.first(dataset.files).href, dataset.author.uid,
    function(err, owner) {
      _.each(dataset.files, function(file) {
          ...
          // We need to use absolute HREFs in our case (CVL).
          // We are serving up data files with Apache on 
          // a different port from the atom feed, so data 
          // file links will include the protocol (https)
          // and the server's IP address.

          // Normally we would give Apache a relative
          // path to the data file, starting at the
          // DocumentRoot, specified in Apache's
          // configuration.  But due to complications
          // from hosting multiple atom feeds on one
          // server, we are using the absolute path to
          // the data file, e.g. /mnt/share/dir/datafile
          // but "mnt/share" is just a symbolic link within
          // the Apache DocumentRoot, pointing back to the
          // DocumentRoot. This way, we don't need to 
          // remove the Apache document root from file.href.

          // encodeURI does not encode the following: , / ? : @ & = + $ # 
          // whereas encodeURIComponent does.

          var pathComponents = file.href.split(path.sep);
          uriEncodedPath = ""
          for (var i in pathComponents) {
            if (pathComponents[i] != "") {
              uriEncodedPath = uriEncodedPath + path.sep + encodeURIComponent(pathComponents[i]);
            }
          }

          file.href = "https://" + getIpAddress() + uriEncodedPath;
      });
