...
var buildDatasetObject = function(details) {
  ...
  var filepath = details['file'].trim();
  // Remove filename from file path:
  var subdirectory = filepath.substring(0,filepath.lastIndexOf("/"));
  // Remove leading directories from file path:
  match = details['groupPattern'].exec(filepath);
  if (match)
  {
    matchedString = details['groupPattern'].exec(filepath)[0];
    subdirectory = subdirectory.substring(matchedString.length);
  }
  else
  {
    subdirectory = "";
  ...
  getOwnerOfFile(_.first(dataset.files).href, dataset.author.uid,
    function(err, owner) {
      _.each(dataset.files, function(file) {
          ...
          file.href =
            convertWindowsPaths(path.relative(directory, uriEncodedPath));
      });
...
var walkDir = function(dir, excludePattern, options, done) {
    ...
    list.forEach(function(file) {
      ...
      fs.stat(file, function(err, stat) {
        if (stat && stat.isDirectory()) {
          walkDir(file, excludePattern, options, function(err, res) {
          ...
          if (!excludePattern.exec(file)) 
            ...
            results[file] = { 'file': file, 'stat': stat, 'groupPattern': options.groupPattern }; // JW
          if (!--pending) 
            done(null, results);
        }
      });
    ...
    //console.log(filenames);
     _.each(filenames, function (f) { 
      results[f] = { 
        'file': f, 
        'stat': fs.statSync(f),
        'groupPattern': options.groupPattern // James W
      };
    });
    process_results(results);
  } else {

    walkDir(directory, options.excludePattern, options, function(err, results) {
      if (err) {
