...
var Options = function() {
   return (new commander.Command)
    ...
    .option('-t, --timeout <timeout>', '<timeout> for web requests to this server in seconds', parseInt, 600)
    .option('--username <username>', '<username> for basic auth', '')
    .option('--password <password>', '<password> for basic auth', '')
    .option('-f, --file-list <filename>', '<filename> containing list of recently created/modified files, oldest to newest.')
    .option('--template <template>', '<template> for feed (path to feed.atom)', '');
}
